    ...
    $.post('../api/sensor/service/search/', params)
      .then((response) => {
        ...
        toastr.error('Request Timeout', 'Notification', {
          "positionClass": "toast-top-right", "showDuration": "300",
          "hideDuration": "1000",
          "timeOut": "1000",
  ...
  async function Search(parseAwal, parseAkhir, keyword) {
    ...
    $.get('../api/sensor/doc/' + parseAwal + '/' + parseAkhir + '/' + keyword)
      ...
      var top_notifier = aggregation.top_direction.buckets;
        ...
        pieSeries.tooltip.label.interactionsEnabled = true;
        pieSeries.tooltip.keepTargetHover = true;
        pieSeries.slices.template.tooltipHTML = '<a class="text-white" data-bs-toggle="modal" data-bs-target="#datalist" data-id="{category}"><b>{category}</b></a>';
