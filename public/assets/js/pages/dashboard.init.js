$(document).ready(function () {
  ...
  async function ModalTables(gte, lte, id) {
    $("<div class="lds-facebook count-total"><div></div><div></div><div></div></div>").show();
    var search_data = Array();
    var params = { gte: gte, lte: lte, keyword: id };
    $.post("../api/sensor/search/", params).then((response) => {
  ...
  async function Search(parseAwal, parseAkhir, keyword) {
    ...
    $.get(
      ...
      pieSeries.slices.template.tooltipHTML =
        '<a class="text-white" data-bs-toggle="modal" data-bs-target="#datalist"><b>{category}</b></a>';
